...
## Design-system fix — map page: white body, gradient only on the header

The desktop `/workspace-map` used the pink→indigo gradient as a fixed full-page background, so listing cards and the "Unlock the special offer" card were translucent tints of it and the pink competed with map pins and CTA buttons. Now: the header keeps `bgMapPageGradient` (the white logo and white-bordered search fields need a dark backdrop), the page body is `bg-white`, and the cards are solid brand colors (`bg-brand-indigo` for the offer card, `bg-brand-navy-muted` for listings). Same rule applied to the shared `Navbar`/`Footer` (white logo on `brand-indigo`, `brand-navy` footer) and the promo bands (`bgPromoGradient` is now navy→blue-light, no pink, so it never blends into the pink→indigo CTA button).
